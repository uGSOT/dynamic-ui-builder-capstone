...
describe("SplitImageRight", () => {
  ...
  it("does not render an image when imageUrl is set to empty", () => {
    render(<SplitImageRight imageUrl="" />);
    expect(screen.queryByRole("img")).not.toBeInTheDocument();
  });

  it("renders a custom headline when passed", () => {
    render(<SplitImageRight headline="Build faster. Launch sooner." />);
    expect(screen.getByRole("heading", { level: 1 })).toHaveTextContent(
  ...
  it("renders custom subtext when passed", () => {
    render(<SplitImageRight subtext="A faster way to ship your product." />);
    expect(
  ...
  it("renders an image when imageUrl is passed", () => {
    render(<SplitImageRight imageUrl="/assets/screenshot.png" />);
  ...
  it("renders a custom badge when passed", () => {
    render(<SplitImageRight badge="14-day free trial" />);
    expect(screen.getByText("14-day free trial")).toBeInTheDocument();
  });

  it("does not render a badge when badge is explicitly empty", () => {
    render(<SplitImageRight badge="" />);
    expect(
