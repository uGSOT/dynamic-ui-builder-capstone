...
export const propSchema = {
  ...
  styles: TEAM_BIO_STYLE_PROP_SCHEMA,
};
        ...
        <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {members.map((member, index) => (
            <div
              ...
              className={`p-6 transition-all hover:shadow-lg ${cardClass}`}
            >
              <div className="mb-4 flex items-start gap-4">
                <div className="h-16 w-16 shrink-0 overflow-hidden rounded-full bg-surface-muted ring-2 ring-border/20">
                  <img
                    src={member.avatar}
                    alt={member.name}
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className={`mt-0.5 font-medium ${roleClass}`}>{member.role}</p>
                </div>
              </div>

              {member.bio ? (
                <p className={`mb-4 leading-relaxed ${bioClass}`}>{member.bio}</p>
              ) : null}

              {member.links && member.links.length > 0 ? (
                <div className="mt-4 flex gap-2 border-t border-border/30 pt-4">
                  {member.links.map((link, i) => (
                    ...
                    >
                      <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24">
                        {link.type === "twitter" && (
                          <path d="M23 3a10.9 10.9 0 01-3.14 1.53 4.48 4.48 0 00-7.86 3v1A10.66 10.66 0 013 4s-4 9 5 13a11.64 11.64 0 01-7 2s9 5 20 0a10.664 10.664 0 00-9.12-5.5z" />
                        )}
                        {link.type === "linkedin" && (
